...
from app.memory import RedisPersistence, UserMemoryService
...
redis_persistence = RedisPersistence.from_env()
user_memory_service = UserMemoryService(redis_persistence.store)
configure_user_memory_service(user_memory_service)
main_agent = None
_main_agent_lock: asyncio.Lock | None = None


def build_main_agent(persistence: RedisPersistence):
    """Build the main agent with the shared Redis persistence components."""

    if persistence.checkpointer is None:
        raise RuntimeError("Redis 异步 checkpoint 尚未初始化")
...
async def initialize_main_agent():
    """Build the graph after the async Redis checkpointer is ready."""

    global main_agent, _main_agent_lock
    ...
    async with _main_agent_lock:
        if main_agent is None:
            main_agent = build_main_agent(redis_persistence)
    return main_agent
...
async def run_deep_agent(task_query, session_id, user_id=None):
    ...
    4. 把 session_dir、session_id 和 user_id 写入 ContextVar
    ...
    :param user_id: 认证用户 ID，用于隔离长期记忆
    """
    started_at = time.perf_counter()
    task_error_type: str | None = None
    log_event("agent_task_started", user_id=user_id, thread_id=session_id)
    ...
    user_id_token = set_user_id_context(user_id)
    ...
    try:
        ...
        reset_user_id_context(user_id_token)
